...
from exammaker.models import Criterion, Difficulty, Item
...
def make_item(**overrides) -> Item:
    defaults = dict(
        ...
        points=10,
        courses={"CS201": Difficulty.MEDIUM, "CS301": Difficulty.EASY},
        criteria=[
...
class TestItem:
    ...
    def test_different_difficulties_per_course(self):
        item = make_item()
        assert item.courses["CS201"] == Difficulty.MEDIUM
        assert item.courses["CS301"] == Difficulty.EASY
    ...
    def test_invalid_difficulty_rejected(self):
        with pytest.raises(ValidationError):
            make_item(courses={"CS201": "impossible"})
